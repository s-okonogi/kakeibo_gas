class Model extends Database {
  ...
  getData(sheetName, conditions, offsetRow, limitRow) {
    const dataList = super.databaseGet(
      this.spreadsheetId,
      sheetName,
      conditions,
      offsetRow,
  ...
  orGetData(sheetName, conditions, offsetRow, limitRow) {
    const dataList = super.databaseOrGet(
      this.spreadsheetId,
      sheetName,
      conditions,
      offsetRow,
  ...
  insertData(sheetName, keyValuePairs) {
    const result = super.databaseInsert(
      this.spreadsheetId,
      sheetName,
      keyValuePairs,
    ); // データの挿入
  ...
  updateData(sheetName, keyValuePairs, conditions) {
    const result = super.databaseUpdate(
      this.spreadsheetId,
      sheetName,
      keyValuePairs,
      conditions,
  ...
  orUpdateData(sheetName, keyValuePairs, conditions) {
    const result = super.databaseOrUpdate(
      this.spreadsheetId,
      sheetName,
      keyValuePairs,
      conditions,
  ...
  deleteData(sheetName, conditions) {
    const result = super.databaseDelete(
      this.spreadsheetId,
      sheetName,
      conditions,
    ); // データの削除
  ...
  orDeleteData(sheetName, conditions) {
    const result = super.databaseOrDelete(
      this.spreadsheetId,
      sheetName,
      conditions,
    ); // データの削除
