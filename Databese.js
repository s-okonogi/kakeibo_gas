class Database {
  ...
  databaseGet(spreadsheetId, sheetName, conditions, offsetRow, limitRow) {
    try {
      ...
      limitRow =
        !limitRow || offsetRow + limitRow >= filteredValues.length
          ? (limitRow = filteredValues.length)
          : offsetRow + limitRow; // 取得個数が未設定またはデータ数以上の場合はデータ数に置き換え
    ...
    try {
      ...
      for (let i = 0; i < keyValuePairs.length; i++) {
        ...
        let newRow = headers.map(function (header) {
          return keyValuePairs[i].hasOwnProperty(header)
            ? keyValuePairs[i][header]
            : null;
        });
        // データの挿入
      ...
      for (let i = 0; i < filteredValues.length; i++) {
        ...
        if (row[keyIndex] === keyValuePair["key"]) {
          for (let updateKey in keyValuePair) {
            ...
            if (updateIndex !== -1) {
              sheet
                .getRange(values.indexOf(row) + 1, updateIndex + 1)
      ...
      values.forEach((row, index) => {
        let meetsConditions = filterConditions.every((condition) => {
          ...
          if (key.endsWith(this.operators.graterThan)) {
            ...
            return (
              columnIndex !== -1 && row[columnIndex] && row[columnIndex] > value
            );
